...
function useHighlightedRef<T extends HTMLElement>() {
  const { setHighlightedValue } = useDropdownMenu();
  const handleHighlightedChange = React.useCallback(
    (value: DataStateValue, element: T | null) => {
      if (value !== true) return;

      const highlightedValue =
        element?.dataset.value || element?.id || null;
      if (highlightedValue) setHighlightedValue(highlightedValue);
    },
    [setHighlightedValue],
  );
  const [, highlightedRef] = useDataState<T>(
    'highlighted',
    undefined,
...
function DropdownMenuSubTrigger({
  ...
  const highlightedRef = useHighlightedRef<HTMLDivElement>();
